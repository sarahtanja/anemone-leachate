---
title: Single stressor respirometry (photosynthesis & respiration) with LoLinR
author: "AS Huffmyer"
date: '2020'
output:
  html_document:
...
Generate empty respiration & photosynthesis data frames.
...
```{r, warning=FALSE, message=FALSE}
# information on leachate treatment exposure, anemone id, and channel
metadata <- read.csv("metadata/resp-metadata.csv") 
```

Subset for respiration data by filtering time `delta_t` \<= 30, since the first 30 minutes were in the dark, with dark adapted anemones. Also remove first 5 minutes of each run to allow for temp equilibration.

```{r}

# for every file in list start at the first and run this following function
...
meanPblank<-mean(blank_data$P.umol.min) #mean P phase blanks
```
...
Run 2 on 20210723 had both blanks removed as they were strong outliers. REspiration rates were not significantly greater in this run than on Run 1 on this day. Therefore, we will take the average respiration blank value and use this for corrections for this run.

```{r}
oxygen$code<-paste(oxygen$Date, oxygen$Run)
plot(oxygen$R.umol.min~as.factor(oxygen$code), ylab="umol O2 min-1")
plot(oxygen$P.umol.min~as.factor(oxygen$code), ylab="umol O2 min-1")
```
...
Add blank values into data frame and add average blank value to replace NA values from 20210723 Run 2.

```{r}
oxygen<-oxygen%>%
  mutate(R.Blank.umol.min = ifelse(is.na(R.Blank.umol.min), meanRblank, R.Blank.umol.min))%>%
  mutate(P.Blank.umol.min = ifelse(is.na(P.Blank.umol.min), meanPblank, P.Blank.umol.min))
```
