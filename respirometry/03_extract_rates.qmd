...
```{r}
print(bg_resp_mean[["rate.bg.mean"]])

resp_run_1.2 <- resp_run_1 %>% select(1,2) %>% drop_na()
inspected <- inspect(resp_run_1.2, time = "delta_t", oxygen = 2)
auto_rated <- auto_rate(inspected)

auto_rated$summary$rate[1]
```
