...
for (i in 1:11) {
  ...
  for (j in 2:6) {
    ...
    calc_rated <- calc_rate(inspected, 6, 25)
...
```{r}
hist(resp_rates$resp_bg_adj.rate)
```

```{r}
hist(resp_rates$calc_resp_rate)
```

```{r}
hist(resp_rates$calc_bg_adj)
```
...
blank <- paste0('resp_bg$', this.run)
new.rate <- new.rate$summary$rate[1]
new.rsq <- new.rate$summary$rsq[1]
new.bg <- adjust_rate(new.rate, by = blank, method = "mean")
new.bg.r2 <- 
  
# Add qaqc value to new column based on conditions in 'run' and 'channel' columns
resp_rates <- resp_rates %>%
  mutate(qaqc_rate = case_when(
    run == this.run & channel == this.channel ~ new.rate)) %>% 
  mutate(qaqc_r2 = case_when(
    run == this.run & channel == this.channel ~ new.rsq)) %>% 
  mutate(qaqc_bg = case_when(
    run == this.run & channel == this.channel ~ new.bg)) %>% 
  mutate(qaqc_bg_r2 = case_when(
    run == this.run & channel == this.channel ~ new.rsq))

filter(resp_rates, run == this.run & channel == this.channel) %>% select("run", "channel", "id", "treatment", "auto_resp_rate", "auto_resp_r2", "qaqc_rate", "qaqc_r2", "qaqc_bg", "qaqc_bg_r2")
```
