...
merged_metadata <- full_join(nem_metadata, resp_log, by = "id")
...
new.bg.adj.rate <- new.adj$summary$rate[1]
# from the background-adjusted object, pull the rsq of the rate
new.bg.adj.r2 <- new.adj$summary$rsq[1]
...
resp_rates <- resp_rates %>%
  mutate(
    ...
    qaqc_bg_adj_r2 = ifelse(run == this.run &
                          channel == this.channel, new.bg.adj.r2, NA)
  )



filter(resp_rates, run == this.run & channel == this.channel) %>% select("run", "channel", "id", "treatment.x", "qaqc_rate", "qaqc_r2", "qaqc_bg_adj_rate", "qaqc_bg_adj_r2")
```
