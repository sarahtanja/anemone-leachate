...
For this I'd like the names of each column to represent the run and the
channel, like `1.1` would represent run 1, channel 1

OR

Should I have each one be the `id` of the anemone? the problem with that
is there are 11 'blanks' that are all the same. Each blank matches a
run, so it could be run.id, or even run.channel.id
...
This is how Craig did it: \## Use `pivot wider` to make dataframes for
each run
...
Our data follows the [Case 3: Mulitple speciment chambers and a single
