...
with the respirometry log later on in the script. Here, we make the
`run` column a factor.
...
or “blank” control experiments with no specimens to quantify it.
...
Use it to get the background for each resp_run
...
=======
>>>>>>> 3a8d0a2 (-already saved changes)
## For photosynthesis

The following code should result in a dataframe that has one row for
each respirometry chamber (in this dataset, 66 rows)
