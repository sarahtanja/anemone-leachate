...
for (i in 1:11) {
  ...
  stored_rates <- data.frame(run = integer(), 
                        ...
                        #calc_resp_rate = numeric(),
                        #calc_resp_r2 = numeric()
  )
  ...
  for (j in 2:6) {
    ...
    # pass object `inspected` through respR `calc_rate()`
    calc_rated <- calc_rate(inspected, 5, 25)
    
    # calculate adjusted rate based on run background/blank
    auto_bg_adj <- adjust_rate(auto_rated, by = get(blank), method = "mean")
    calc_bg_adj <- adjust_rate(calc_rated, by = get(blank), method = "mean")
    ...
    stored_rates_channels <- data.frame(
      ...
      #calc_resp_rate = calc_rated$summary$rate[1],
      #calc_resp_r2 = calc_rated$summary$rsq[1],
      resp_bg_adj.rate = auto_bg_adj$summary$rate.adjusted[1],
      resp_bg_r2 = auto_bg_adj$summary$rsq[1]
      #calc_bg_adj = calc_bg_adj$summary$rate.adjusted[1],
      #calc_bg_r2 = calc_bg_adj$summary$rsq[1]
    )
...
Why is this different than the dataframe ... UGH WAIT is it because I
filtered out the NAs in the dataframe, but not here ?!
...
I don't know why... but I want to use the data from the non NA dropped
one in this case, so I'm going to replace it in the `resp_rates`
dataframe
...
```{r}
inspect(resp_run_2, time = 'delta_t', oxygen = '2') %>% auto_rate() 
```
...
this.run <- 2
this.channel <- 2
new.rate <- new2.2$summary$rate[1]
new.rsq <- new2.2$summary$rsq[1]
...
```{r}
inspect(resp_run_2, time = 'delta_t', oxygen = '4') %>% auto_rate() 
```
...
this.run <- 2
this.channel <- 4
new.rate <- new2.4$summary$rate[1]
new.rsq <- new2.4$summary$rsq[1]
...
```{r}
inspect(resp_run_6, time = 'delta_t', oxygen = '3') %>% auto_rate() 
```
...
this.run <- 6
this.channel <- 3
new.rate <- new6.3$summary$rate[1]
new.rsq <- new6.3$summary$rsq[1]
...
```{r}
inspect(resp_run_9, time = 'delta_t', oxygen = '7') %>% auto_rate()
```
...
this.run <- 9
this.channel <- 7
new.rate <- new9.7$summary$rate[1]
new.rsq <- new9.7$summary$rsq[1]
...
```{r}
inspect(resp_run_10, time = 'delta_t', oxygen = '4') %>% auto_rate()
```
...
this.run <- 10
this.channel <- 4
new.rate <- new10.4$summary$rate[1]
new.rsq <- new10.4$summary$rsq[1]
...
```{r}
inspect(resp_run_10, time = 'delta_t', oxygen = '7') %>% auto_rate()
```

no changes needed on this one, the regression line looks good. I'm not
sure why it was flagged
...
this.run <- 10
this.channel <- 7
new.rate <- new10.7$summary$rate[1]
new.rsq <- new10.7$summary$rsq[1]
...
```{r}
inspect(resp_run_10, time = 'delta_t', oxygen = '1') %>% auto_rate()
```

```{r}
resp_run_10 %>%
  inspect(time = 'delta_t', oxygen = '1') %>%
  calc_rate(from = 5, to = 15, by = 'time')
```
...
new10.1 <-  resp_run_10 %>%
  inspect(time = 'delta_t', oxygen = '1') %>%
  calc_rate(from = 5, to = 15
            , by = 'time')

this.run <- 10
this.channel <- 1
new.rate <- new10.1$summary$rate[1]
new.rsq <- new10.1$summary$rsq[1]
...
```{r}
inspect(resp_run_10, time = 'delta_t', oxygen = '5') %>% auto_rate()
```
...
this.run <- 10
this.channel <- 5
new.rate <- new10.5$summary$rate[1]
new.rsq <- new10.5$summary$rsq[1]
...
```{r}
inspect(resp_run_10, time = 'delta_t', oxygen = '2') %>% auto_rate()
```
...
this.run <- 10
this.channel <- 2
new.rate <- new10.2$summary$rate[1]
new.rsq <- new10.2$summary$rsq[1]
...
for (i in 1:11) {
  ...
  for (j in 2:6) {
    ...
    # pass object `inspected` through respR `calc_rate()`
    calc_rated <- calc_rate(inspected, 40, 60)
    ...
    stored_rates_channels <- data.frame(
      ...
      #calc_resp_rate = calc_rated$summary$rate[1],
      #calc_resp_r2 = calc_rated$summary$rsq[1],
      phot_bg_adj.rate = auto_bg_adj$summary$rate.adjusted[1],
      phot_bg_r2 = auto_bg_adj$summary$rsq[1]
      #calc_bg_adj = calc_bg_adj$summary$rate.adjusted[1],
      #calc_bg_r2 = calc_bg_adj$summary$rsq[1]
    )
...
resp_rates <- resp_rates %>% select("id", "treatment", "run", "channel", "resp_rate", "resp_r2", "phot_rate", "phot_r2", "wet_weight_(g)")
```
...
```{r}
rates_select_cols <- rates_all_cols %>% select("id", "treatment", "run", "channel", "resp_rate", "resp_r2", "phot_rate", "phot_r2", "wet_weight_(g)")

head(rates_select_cols)

```

# Adjust rates by blanks

Following the examples detailed in the [adjust_rate: Adjusting rates for
...
```{r}
resp_rates$bg_adj_rate <- adjust_rate(resp_rates , by = bg_resp_run_1, method = "mean")
```
